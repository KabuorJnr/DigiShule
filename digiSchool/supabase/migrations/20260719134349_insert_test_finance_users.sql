...
BEGIN
    ...
    SELECT id, split_part(email, '@', 1), raw_user_meta_data->>'name', (CASE WHEN raw_user_meta_data->>'role' = 'accountant' THEN 'bursar' ELSE raw_user_meta_data->>'role' END)::public.app_role, id, (SELECT id FROM public.schools LIMIT 1)
    FROM auth.users
    WHERE email IN ('bursar@digischool.com', 'accountant@digischool.com')
