...
// Presentational A4 report-card sheet (794 x 1123 px). Shared by the on-screen
// ReportCardModal and the bulk PDF exporter so every portal and every download
// produces a pixel-identical, verifiable card.
export default function ReportCardSheet({ report, student = {}, schoolSettings = {}, captureId = 'report-card-capture-area' }) {
            ...
            <div style={{ position: 'relative', height: 80, borderBottom: '1px solid #e2e8f0', borderLeft: '1px solid #e2e8f0' }}>
              <svg viewBox={`0 0 ${subjectsGraphData.length * 40} 100`} preserveAspectRatio="none" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
                ...
                {subjectsGraphData.map((d, i) => (
                  <circle key={i} cx={i * 40 + 20} cy={100 - d.score} r="3" fill="#b8860b" />
                ))}
              </svg>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', border: `1px solid #cbd5e1`, marginBottom: 14 }}>
          ...
          <tbody>
            {report.subjectRows.map(row => {
              const dev = (row.score || 0) - 65; // Deviation from a 65% baseline
